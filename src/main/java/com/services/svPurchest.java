 ...
 * @author amirk
 */
@WebServlet(name = "svPurchest", urlPatterns = {"/svPurchest"})
public class svPurchest extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        rs = om.result("SELECT u.user_s_c_t_s_c_t_id,i.fname,i.lname,u.registration_date,s.title,s.description,c.name,u.global_status_id,u.user_s_c_t_id FROM `user_s_c_t` as u inner join"
                ...
                + " user as us on s.`teacher_id` = us.`user_id` inner join user_information as i on us.`user_information_id` = i.`user_information_id` where u.registration_date + interval 365 day >= NOW() AND `user_sub_course_user_id` =" + _id +" order by u.user_s_c_t_s_c_t_id DESC");
        try {
            _o = "[";
            while (rs.next()) {
