 ...
 * @author ryan
 */
@WebServlet(name = "mcallback", urlPatterns = {"/mcallback"})
public class mcallback extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (Integer.parseInt(b) == 0) {
                if (Integer.parseInt(doBanck(b, c, d)) == 0) {
                    ...
                    doDB(c, a);
                    response.sendRedirect("/Error/bank_successful_m.xhtml");

                } else {
                    response.sendRedirect("/Error/bank_error_m.xhtml");
                }
