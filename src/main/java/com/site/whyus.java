...
public class whyus {
    ...
    public void updateRecord() {
        Model om = new Model();
        boolean res = om.insert("INSERT INTO `lms`.`web_why_us` (`w_id`, `w_title`, `w_desciption`, `w_web`) VALUES (NULL, '" + title + "', '" + description + "', '1');");
        if (res) {
    ...
    public void content() throws SQLException {
        Model om = new Model();
        ResultSet rs = om.result("SELECT * FROM `web_why_us` order by `w_id` DESC limit 1");
        while (rs.next()) {
