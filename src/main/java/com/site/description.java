...
public class description {
    ...
    public void updateRecord() {
        Model om = new Model();
        boolean res = om.insert("INSERT INTO `lms`.`web_description` (`d_id`, `d_title`, `d_content`, `d_web`) VALUES "
    ...
    public void content() throws SQLException {
        Model om = new Model();
        ResultSet rs = om.result("SELECT * FROM `web_description` order by `d_id` DESC limit 1");
        while (rs.next()) {
